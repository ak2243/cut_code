...
public class GraphicalReturnBlock extends GraphicalBlock {
	...
	public GraphicalReturnBlock(double width, double height) { //sets up visuals of the block
		super(width, height);
	...
	public LogicalBlock getLogicalBlock() throws BlockCodeCompilerErrorException {
		LogicalBlock ret = null;
		if(nestBoxes[0].getChildren().size() > 0) {
			ret = ((GraphicalBlock)nestBoxes[0].getChildren().get(0)).getLogicalBlock();
		}
		return logicalFactory.createReturnBlock(getIndentFactor(), ret);
	}

	@Override
	public GraphicalBlock cloneBlock() {
		return new GraphicalReturnBlock(initWidth, initHeight);
	}
