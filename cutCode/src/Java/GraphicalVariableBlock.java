...
public class GraphicalVariableBlock extends GraphicalBlock {
	...
	public GraphicalVariableBlock(double width, double height) {
		...
		this.setBackground(new Background(new BackgroundFill(Color.web("#D097F4"), CornerRadii.EMPTY, Insets.EMPTY)));
	...
	public Point2D[] getNestables() { // returns the top left of the nest point
		...
		for (int i = 0; i < nestBoxes.length; i++)
			ret[i] = nestBoxes[i].localToScene(nestBoxes[i].getLayoutBounds().getMinX(),
					nestBoxes[i].getLayoutBounds().getMinY());
		return ret;
	}

	@Override
	public void nest(int index, GraphicalBlock nest) throws InvalidNestException {
		if (nestBoxes[index].getChildren().size() != 0) // can only nest one block inside it (this block can have blocks
														// nested inside it though)
	...
	@Override
	public void unnest(VBox box, GraphicalBlock rem) throws InvalidNestException { // takes out the block and resets
	...
	public LogicalBlock getLogicalBlock() throws BlockCodeCompilerErrorException {
		...
		return logicalFactory.createVariable(getIndentFactor(), types.getValue(), name.getText(),
				((GraphicalBlock) nestBoxes[0].getChildren().get(0)).getLogicalBlock());
	}
